...
### Active Phase
Landing-motion sweep in flight alongside the P.5 storage swap. The sweep is three sequential PRs, all off `develop`: (PR 1) `refactor/landing-consume-motion-tokens` did a mechanical swap of every hardcoded `0.15s` / `0.2s` / `0.25s` / `ease` / `ease-in-out` in `playground/src/pages/Landing.module.css` for the semantic motion tokens (`var(--duration-fast|normal|slow)` / `var(--ease-default|out|in-out)`) and added a bullet to `CONTRIBUTING.md` saying landing and playground pages must use motion tokens; (PR 2, this branch `feat/landing-enterprise-motion`) introduces four token-driven motion primitives (`FadeIn`, `Stagger`, `CountUp`, `GradientBorder`) under `playground/src/components/motion/` plus the shared `useInView` hook under `playground/src/hooks/`, threads them through every section of the landing (hero fades 0/100/200/300/400ms, logo-cloud stagger 60ms, features stagger 80ms wrapped in `GradientBorder`, how-it-works stagger 120ms, themes stagger 40ms, showcase fade `translateY=48`, stats `CountUp` 108/2600/14/5, CTA cursor-tracked radial spotlight), adds two ambient drift blobs behind the hero, and gives `@arcana-ui/core` Buttons a `scale(0.98)` on `:active` keyed to `var(--duration-fast)` + `var(--ease-out)`; (PR 3, not yet cut) will push per-preset motion personality overrides into each `packages/tokens/src/presets/*.json`, add a Motion section to the Token Editor with duration sliders + a `CubicBezierEditor` picker, extend the manifest pipeline to capture motion personality, add a Motion Personality column to the ROADMAP preset matrix, and add a Playwright harness that sets `animation-delay: -999s` so visual snapshots can actually baseline the reveal effects.
